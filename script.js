...
>>>>>>> 1a22452086e0f247f3bd974197884ee7924ecd43
function checkUserLogin(){
const currentUser = localStorage.getItem("currentUser");
if(currentUser){
showMainApp();
...
// Use localhost for local dev, and your production URL for cloud deployment
const API_BASE_URL = (window.location.hostname === "127.0.0.1" || window.location.hostname === "localhost")
    ? "http://127.0.0.1:8000"
    : "https://certification-hub.onrender.com"; // Updated production URL
...
function updateUserProfile(){
    ...
    if (currentUser.role) {
        ...
        if (roleVal) roleVal.textContent = currentUser.role;
        if (adminLink) {
            adminLink.style.display = currentUser.role.toLowerCase() === "admin" ? "block" : "none";
        }
    } else {
...
try {
    const response = await fetch(`${API_BASE_URL}/certificates/`, {
        ...
        body: JSON.stringify({ name, issuer, date, link, photo: null })
    });
...
if (token) {
    try {
        ...
        if (response.ok) {
            ...
            localStorage.setItem("certificates_" + currentUser.username, JSON.stringify(certificates));
        }
    } catch (error) {
        console.error("Failed to fetch certificates from backend:", error);
        const currentUser = JSON.parse(localStorage.getItem("currentUser")) || {username: "user"};
        certificates = JSON.parse(localStorage.getItem("certificates_" + currentUser.username)) || [];
    }
} else {
    const currentUser = JSON.parse(localStorage.getItem("currentUser"));
    if (currentUser) {
        const userCertificatesKey = "certificates_" + currentUser.username;
        certificates=JSON.parse(localStorage.getItem(userCertificatesKey)) || [];
    }
}
...
/* ADMIN DASHBOARD - ADVANCED UX */
async function getAdminData() {
    const listContainer = document.getElementById("admin-users-list");
    const certsContainer = document.getElementById("admin-certs-panel");
    if(!listContainer || !certsContainer) return;

    const token = localStorage.getItem("token");
    if (!token) return;

    // Loading states & Animations
    listContainer.innerHTML = '<p style="text-align:center; padding: 30px; color:#666; font-style:italic;">Loading users...</p>';
    ...
    try {
        ...
        if (response.status === 403) {
            showAdminToast("Admin privileges required.", "error");
            return;
        }
        ...
        console.error(error);
        listContainer.innerHTML = '<p style="color:#e74c3c; text-align:center; padding: 20px;">Something went wrong while loading users.</p>';
    }
}
...
function renderAdminDashboard(users) {
    ...
    if (users.length === 0) {
        listContainer.innerHTML = '<p style="text-align:center; padding: 30px; color:#666;">No users found</p>';
        return;
    }
    ...
    users.forEach(user => {
        html += `
            <tr class="row-user" id="admin-user-row-${user.id}" onclick="loadAdminUserCertificates(${user.id}, '${user.username}')">
                <td style="padding:12px;"><strong>${user.username}</strong><br><small style="color:#888;">${user.email || 'No email'}</small></td>
                <td style="padding:12px;"><span class="status-badge ${user.role==='admin' ? 'status-verified' : 'status-pending'}">${user.role}</span></td>
            </tr>
        `;
    });
    html += `</tbody></table>`;
...
async function loadAdminUserCertificates(userId, username) {
    // Highlight Active
    document.querySelectorAll('.row-user').forEach(el => el.classList.remove('active-user'));
    const activeRow = document.getElementById(`admin-user-row-${userId}`);
    if(activeRow) activeRow.classList.add('active-user');

    const certsContainer = document.getElementById("admin-certs-panel");
    certsContainer.style.display = 'block';

    // Smooth reset
    certsContainer.classList.remove('fade-in');
    void certsContainer.offsetWidth; // trigger reflow
    certsContainer.classList.add('fade-in');

    certsContainer.innerHTML = '<p style="text-align:center; padding: 30px; color:#666; font-style:italic;">Loading certificates...</p>';
    ...
    try {
        ...
        if (!response.ok) throw new Error("Failed to fetch user certificates");

        const certificates = await response.json();

        let html = `<h3 style="margin-top:0; color:#333;">Certificates: ${username}</h3>`;

        if (certificates.length === 0) {
            html += `<p style="padding:20px; background:#f9f9f9; border-radius:5px; color:#666; font-style:italic;">No certificates uploaded.</p>`;
        } else {
            html += `
            <table style="width:100%; border-collapse: collapse; text-align: left; margin-top:15px; font-size:14px;">
                <thead>
                    <tr style="border-bottom:2px solid #ddd;">
                        <th style="padding:10px;">Name</th>
                        <th style="padding:10px;">Issuer & Date</th>
                        <th style="padding:10px;">Link/Photo</th>
            ...
            certificates.forEach(cert => {
                let linkHtml = cert.link ? `<a href="${cert.link}" target="_blank" style="color:#667eea; text-decoration:none; display:inline-block; margin-bottom:5px;">🔗 Try Link</a><br>` : '';
                let photoHtml = cert.photo ? `<button onclick="viewCertificatePhoto('${cert.name}', '${cert.photo}')" style="background:#3498db;color:white;border:none;padding:4px 8px;border-radius:3px;cursor:pointer;font-size:12px;">🖼️ Photo</button>` : '';
                if(!cert.link && !cert.photo) linkHtml = '<span style="color:#aaa;">None</span>';

                const currStatus = cert.status || 'pending';
                let disableVerify = currStatus === 'verified' ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : '';
                let disableReject = currStatus === 'rejected' ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : '';

                html += `
                <tr style="border-bottom:1px solid #f0f0f0; transition: background 0.3s;" id="cert-row-${cert.id}">
                    <td style="padding:12px;"><strong>${cert.name}</strong></td>
                    <td style="padding:12px;">${cert.issuer}<br><span style="color:#888;font-size:12px;">${cert.date}</span></td>
                    <td style="padding:12px;">${linkHtml} ${photoHtml}</td>
                    <td style="padding:12px;">
                        <span class="status-badge status-${currStatus}">${currStatus}</span>
                    </td>
                    <td style="padding:12px;">
                        <button ${disableVerify} onclick="updateCertificateStatus(${cert.id}, 'verified', ${userId}, '${username}')" style="background:#2ecc71;color:white;border:none;padding:5px 10px;border-radius:3px;cursor:${currStatus==='verified'?'not-allowed':'pointer'};margin-bottom:4px;width:100%;">Verify</button>
                        <br>
                        <button ${disableReject} onclick="updateCertificateStatus(${cert.id}, 'rejected', ${userId}, '${username}')" style="background:#e74c3c;color:white;border:none;padding:5px 10px;border-radius:3px;cursor:${currStatus==='rejected'?'not-allowed':'pointer'};width:100%;">Reject</button>
                    </td>
                </tr>
                `;
            });
        ...
        console.error(error);
        certsContainer.innerHTML = '<p style="color:#e74c3c; text-align:center;">Something went wrong while loading certificates.</p>';
        showAdminToast("Failed to fetch user certificates", "error");
    }
}

async function updateCertificateStatus(certId, status, userId, username) {
    const token = localStorage.getItem("token");
    if (!token) return;

    try {
        const response = await fetch(`${API_BASE_URL}/certificates/${certId}/status`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        showAdminToast(`Certificate marked as ${status}!`, "success");

        // Auto refresh certificates list smoothly
        loadAdminUserCertificates(userId, username);

    } catch (error) {
        console.error(error);
        showAdminToast("Something went wrong updating status", "error");
    }
}
